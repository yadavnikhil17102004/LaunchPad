...
import { Heart, ArrowLeft, Trash2, ExternalLink, Calendar, MapPin, Trophy, Download, Copy, FileSpreadsheet } from 'lucide-react';
...
const Favorites = () => {
  ...
  const exportToCSV = () => {
    if (favorites.length === 0) {
      toast({ title: "No favorites to export", variant: "destructive" });
      return;
    }

    const headers = ['Title', 'Type', 'Organization', 'Deadline', 'Location', 'Prize', 'Apply URL', 'Tags'];
    const rows = favorites.map(fav => {
      const opp = parseOpportunity(fav.opportunity_data);
      if (!opp) return [];
      return [
        `"${opp.title.replace(/"/g, '""')}"`,
        opp.type,
    ...
    link.download = `opportune-favorites-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();

    toast({
      title: "✅ Exported successfully!",
      description: `${favorites.length} opportunities saved as CSV`
    });
  };

  // Copy for Notion (Markdown table format)
  const copyForNotion = () => {
    if (favorites.length === 0) {
      toast({ title: "No favorites to copy", variant: "destructive" });
      return;
    }

    const header = '| Title | Type | Organization | Deadline | Prize | Link |\n|-------|------|--------------|----------|-------|------|\n';
    const rows = favorites.map(fav => {
      const opp = parseOpportunity(fav.opportunity_data);
      if (!opp) return '';
      return `| ${opp.title} | ${opp.type} | ${opp.organization} | ${opp.deadline.toLocaleDateString()} | ${opp.prize || '-'} | [Apply](${opp.applyUrl}) |`;
    }).filter(row => row).join('\n');

    const notionContent = header + rows;
    navigator.clipboard.writeText(notionContent);

    toast({
      title: "📋 Copied to clipboard!",
      description: "Paste directly into Notion as a table"
    });
  };
              ...
              {favorites.length > 0 && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    ...
                    <DropdownMenuItem onClick={copyForNotion} className="cursor-pointer">
